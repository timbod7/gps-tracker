...
use embedded_graphics::geometry::{Size, Point, OriginDimensions,};
  ...
  PinE: core::fmt::Debug,
{
  ...
  fn toggle_vcom(&mut self) -> u8 {
    self.vcom = !self.vcom;
    if self.vcom {64} else {0}
  }
}

impl<SPI,CS,DELAY, const W: usize, const H: usize> DrawTarget for  MemoryDisplay<SPI,CS,DELAY,W,H> {
  ...
  fn clear(&mut self, color: Self::Color) -> Result<(), Self::Error> {
    for b in self.framebuf.iter_mut() {
      *b = if color == BinaryColor::Off {0xff} else {0x00};
    }
...
impl<SPI,CS,DELAY, const N: usize, const H: usize> OriginDimensions for MemoryDisplay<SPI,CS,DELAY,N,H> {
  fn size(&self) -> Size {
